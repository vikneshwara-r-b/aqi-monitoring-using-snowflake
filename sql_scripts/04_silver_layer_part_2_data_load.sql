...
-- Step-1: View the records for a single pollution board station - Bangalore/Silkboard
select 
...
-- Step-2: View columns having Missing Data (How to handle NA)
select 
    hour(index_record_ts) as measurement_hours,
    * 
from 
...
-- Viewing the records in temporary table
select * from air_quality_tmp

-- Filtering records for 1 pollution station
select 
    hour(INDEX_RECORD_TS) as measurement_hours,
    *
from 
...
SELECT DISTINCT STATE FROM air_quality_tmp
...
create or replace dynamic table curated_flatten_aqi_dt
    target_lag='30 min'
