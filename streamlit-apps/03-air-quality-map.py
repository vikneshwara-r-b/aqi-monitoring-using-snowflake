...
state_query = """
    select state from aqi_monitoring_db.gold.dim_location 
    group by state 
    order by 1
"""
...
if (state_option is not None and len(state_option) > 1):
    city_query = f"""
    select city from aqi_monitoring_db.gold.dim_location 
    where 
...
if (city_option is not None and len(city_option) > 1):
    station_query = f"""
    select station from aqi_monitoring_db.gold.dim_location 
        where 
...
if (station_option is not None and len(station_option) > 1):
    date_query = f"""
    select date(measurement_time) as measurement_date from aqi_monitoring_db.gold.dim_date
        group by 1 
        order by 1 desc
    """
...
if (date_option is not None):
    ...
    select 
        ...
        AQI
    from 
        aqi_monitoring_db.gold.fact_air_quality f 
        join 
        aqi_monitoring_db.gold.dim_date d on d.date_pk  = f.date_fk and date(measurement_time) = '{date_option}'
        join 
        aqi_monitoring_db.gold.dim_location l on l.location_pk  = f.location_fk and 
